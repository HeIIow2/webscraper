...
if __name__ == "__main__":
    if RUN_DATA_PREPARATION_FOR_CLUSTERING:
        ...
        for i, (date, data) in enumerate(data_iter):
            ...
            keywords = extract_keywords.get_keywords(magazines=MAGAZINES, data=data,
                                                     ...
                                                     header_importance_factor=HEADER_IMPORTANCE_FACTOR,
                                                     min_text_header_ratio=MIN_TEXT_HEADER_RATIO)
            for magazine in keywords:
                ...
                keyword_buffer.append({"date": datetime.strftime(date, DATE_FORMAT), "magazine": magazine,
                                       "keywords": keywords[magazine]})
    ...
    if ANALYZE_TIME:
        ...
        for magazine in processing_map:
            dates, data = processing_map[magazine].retrieve_data(
                os.path.join(ANALYZED_PATH, f"{magazine}_analyzed_time.json"), DATE_FORMAT)
    ...
    if DRAW_DIAGRAM:
        for magazine in MAGAZINES:
            ...
            magazine_labels = label_community.Label(magazine, GRAPH_PATH=GRAPH_PATH, ANALYZED_PATH=ANALYZED_PATH,
                                                    modularity_file=MODULARITY_FILE, label_file=LABEL_FILE,
                                                    override_description=OVERIDE_LABEL_DESCRIPTION,
                                                    keywords_in_description=KEYWORDS_IN_DESCRIPTION)

            draw_diagram.draw_diagram(magazine_labels, magazine, dates, data, os.path.join(ANALYZED_PATH,
                                                                                           DIAGRAM_FILE.format(
                                                                                               magazine=magazine,
